...
pub fn varu32_write(buf: &mut Vec<u8>, value: u32) {
    if value <= VARINT_U16_MAX as u32  {
        if value <= VARINT_U8_MAX as u32  {
            buf.push(value as u8);
        } else {
            buf.put_u16(value as u16 | 0b01 << 14);
        }
    } else {
        if value <= VARINT_U32_MAX {
            buf.put_u32(value | 0b10 << 30);
        } else {
...
pub fn varu32_try_read(buf: &[u8], idx: &mut usize) -> Option<u32> {
    ...
    Some(match buf[i] >> 6 {
        ...
        0b01 => {
            ...
            *idx = j;
            (u16::from_be_bytes(buf[i..j].try_into().unwrap()) & VARINT_U16_MAX) as u32
        }
        0b10 => {
            let j = *idx + 4;
            if j > buf.len() {
            ...
            *idx = j;
            u32::from_be_bytes(buf[i..j].try_into().unwrap()) & VARINT_U32_MAX
        }
        0b11 => {
            let j = *idx + 8;
            if j > buf.len() {
            ...
            *idx = j;
            u32::try_from(u64::from_be_bytes(buf[i..j].try_into().unwrap())).ok()?
        }
        _ => unreachable!()
    })
}
